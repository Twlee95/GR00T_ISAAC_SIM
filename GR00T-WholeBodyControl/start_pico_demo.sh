...
pkill -TERM -f "[s]onic_xr_pipeline/sonic_xr_teleop.py" 2>/dev/null || true
...
python3 -u gear_sonic/sonic_xr_pipeline/sonic_xr_teleop.py \
    --task Isaac-PickPlace-Demo-G1-Abs-v0 \
    --xr \
    --device cuda:0 \
    --headless \
    --no-auto_launch_cloudxr \
    --num_demos 0 \
    --dataset_file ./datasets/teleop_demo.hdf5 \
    >> /tmp/isaaclab_teleop.log 2>&1 &

teleop_pid=$!
echo "$teleop_pid" > /tmp/pico_isaac_teleop.pid
echo "Isaac Teleop PID=$teleop_pid"

# 화면 출력용 tail은 Python과 연결하지 않는다.
tail -n 0 -F /tmp/isaaclab_teleop.log &
tail_pid=$!
...
echo "$status" > /tmp/pico_isaac_teleop.exit
