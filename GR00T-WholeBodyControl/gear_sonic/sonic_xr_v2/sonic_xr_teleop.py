...
def _install_sonic_full_body_capture():
    """Add PICO full-body tracking without modifying IsaacLab originals."""
    ...
    if not getattr(TeleopSessionLifecycle, "_sonic_full_body_patched", False):
        original_try_start = TeleopSessionLifecycle._try_start_session

        def try_start_with_full_body(self):
            if (
                self._pipeline is not None
                and not hasattr(self, "_sonic_full_body_source")
            ):
                previous = self._pipeline
                source = FullBodySource(name="_sonic_full_body")

                self._pipeline = OutputCombiner(
                    {
                        "action": previous.output("action"),
                        self._CONTROLLER_RIGHT_KEY: previous.output(
                            self._CONTROLLER_RIGHT_KEY
                        ),
                        "sonic_full_body": source.output(
                            FullBodySource.FULL_BODY
                        ),
                    }
                )
                self._sonic_full_body_source = source
                print(
                    "[SONIC-FULLBODY] FullBodyTrackerPico registered",
                    flush=True,
                )

            return original_try_start(self)

        TeleopSessionLifecycle._try_start_session = try_start_with_full_body
        TeleopSessionLifecycle._sonic_full_body_patched = True

    if not getattr(TeleopSession, "_sonic_result_capture_patched", False):
        original_session_step = TeleopSession.step

        def step_and_capture(self, *args, **kwargs):
            result = original_session_step(self, *args, **kwargs)
            self._sonic_last_result = result
            return result

        TeleopSession.step = step_and_capture
        TeleopSession._sonic_result_capture_patched = True
...
def setup_teleop_device(callbacks: dict[str, Callable], use_isaac_teleop: bool = False) -> object:
    ...
    try:
        if use_isaac_teleop:
            _install_sonic_full_body_capture()
...
def run_simulation_loop(
    ...
    state_socket.connect(
        f"tcp://{args_cli.sonic_state_host}:"
        f"{args_cli.sonic_state_port}"
    )

    # Publish official PICO full-body tracking to SONIC.
    ...
    pose_pub_count = 0
    pose_poll_count = 0
    print("[SONIC-FULLBODY] publishing tcp://*:5566", flush=True)

    state_robot = env.scene["robot"]
    body_joint_names = [
    ...
    mirror_step = 0
    official_root_state = None

    print(
        "[SONIC-MIRROR] waiting for "
        f"tcp://{args_cli.sonic_state_host}:"
        f"{args_cli.sonic_state_port}",
        flush=True,
    )
    ...
    def inner_loop():
        ...
        nonlocal mirror_step, official_root_state
        nonlocal pose_pub_count, pose_poll_count

        # Reset before starting
        env.sim.reset()
        env.reset()

        initial_root = state_robot.data.root_state_w
        if hasattr(initial_root, "torch"):
            initial_root = initial_root.torch
        official_root_state = initial_root.clone()
        ...
        with contextlib.suppress(KeyboardInterrupt), torch.inference_mode():
            while simulation_app.is_running():
                ...
                if (
                    ...
                    and not getattr(
                        teleop_interface,
                        "_sonic_full_body_reported",
                        False,
                    )
                ):
                    lifecycle = teleop_interface._session_lifecycle
                    ...
                    if result is not None:
                        full_body = result.get("sonic_full_body")
                        print(
                            "[SONIC-FULLBODY] keys="
                            f"{list(result.keys())} "
                            f"type={type(full_body)} "
                            f"value={full_body!r}",
                            flush=True,
                        )
                        teleop_interface._sonic_full_body_reported = True
                ...
                if use_isaac_teleop:
                    ...
                    session = lifecycle._session
                    result = (
                        getattr(session, "_sonic_last_result", None)
                        if session is not None else None
                    )
                    full_body = (
                        result.get("sonic_full_body")
                        if result is not None else None
                    )

                    if full_body is not None and not full_body.is_none:
                        ...
                        orientations = np.asarray(
                            full_body[
                                FullBodyInputIndex.JOINT_ORIENTATIONS
                            ],
                            dtype=np.float32,
                        )
                        valid = np.asarray(
                        ...
                        if (
                            positions.shape == (24, 3)
                            and orientations.shape == (24, 4)
                            and valid_count == 24
                        ):
                            positions_isaac = (
                                openxr_to_isaac @ positions.T
                            ).T
                            pelvis = positions_isaac[0].copy()

                            root_openxr = quat_xyzw_to_matrix(
                                orientations[0]
                            )
                            root_isaac = (
                                openxr_to_isaac
                            ...
                            # Same convention as smpl_joints_local:
                            # pelvis-relative with global root rotation removed.
                            smpl_joints = (
                                root_isaac.T
                                @ (positions_isaac - pelvis).T
                            ).T.astype(np.float32)

                            root_quat = matrix_to_quat_wxyz(
                                root_isaac
                            )
                            ...
                            pose_socket.send(message)
                            pose_pub_count += 1

                            if pose_pub_count % 100 == 0:
                                print(
                                    "[SONIC-FULLBODY] "
                                    f"published={pose_pub_count} "
                                    f"valid={valid_count}/24",
                                    flush=True,
                                )
                        elif pose_poll_count % 100 == 0:
                            nonzero_joints = int(
                                np.count_nonzero(
                                    np.linalg.norm(positions, axis=1) > 1.0e-6
                                )
                            )
                            checksum = float(np.abs(positions).sum())
                            print(
                                "[SONIC-FULLBODY] waiting: "
                                f"positions={positions.shape} "
                                f"valid={valid_count}/24 "
                                f"nonzero={nonzero_joints}/24 "
                                f"checksum={checksum:.6f} "
                                f"pelvis={positions[0].tolist()} "
                                f"head={positions[15].tolist()}",
                                flush=True,
                            )
                    elif pose_poll_count % 100 == 0:
                        print(
                            "[SONIC-FULLBODY] body tracking inactive",
                            flush=True,
                        )
                ...
                try:
                    while True:
                        latest_state = state_socket.recv_pyobj(
                            flags=zmq.NOBLOCK
                        )
                except zmq.Again:
                    pass
                ...
                received_joint_pos = torch.as_tensor(
                    latest_state["joint_pos"],
                    dtype=torch.float32,
                    device=env.device,
                ).unsqueeze(0)
                received_joint_vel = torch.as_tensor(
                    latest_state["joint_vel"],
                    dtype=torch.float32,
                    device=env.device,
                ).unsqueeze(0)

                full_joint_pos = tensor_value(
                    state_robot.data.joint_pos
                ).clone()
                full_joint_vel = tensor_value(
                    state_robot.data.joint_vel
                ).clone()

                full_joint_pos[:, state_joint_ids] = received_joint_pos
                full_joint_vel[:, state_joint_ids] = received_joint_vel

                # Keep the official G1 root frame and XR anchor stable.
                state_robot.write_root_state_to_sim(
                    official_root_state
                )
                state_robot.write_joint_state_to_sim(
                    full_joint_pos,
                    full_joint_vel,
                )
                ...
                if mirror_step % 50 == 0:
                    print(
                        f"[SONIC-MIRROR] step={mirror_step}",
                        flush=True,
                    )
                continue
