...
from gear_sonic.sonic_xr_newton.newton14_cubric_compat_patch import (
    install as install_newton14_cubric_compat,
)

install_newton14_cubric_compat()

from gear_sonic.sonic_xr_newton.newton14_independent_xr_anchor_patch import (
    install as install_newton14_independent_anchor,
)

install_newton14_independent_anchor()

from gear_sonic.sonic_xr_newton.newton14_xr_anchor_patch import (
    install as install_newton14_xr_anchor,
)

install_newton14_xr_anchor()
...
# Remaining arguments are forwarded to the SONIC runner.
...
import gear_sonic.sonic_xr_newton.official_task_newton

from gear_sonic.sonic_xr_pipeline.robot_dataset_capture import (
    RobotDatasetRecorderManagerCfg,
    install_robot_dataset_camera,
)
...
def create_environment_config(
    ...
    install_robot_dataset_camera(env_cfg)
    env_cfg.recorders: RobotDatasetRecorderManagerCfg = RobotDatasetRecorderManagerCfg()
...
def handle_reset(
    ...
    # env.reset() 내부의 record_pre_reset이 에피소드를 export한 뒤
    # RecorderManager 버퍼를 초기화한다.
    env.reset()

    manager = env.recorder_manager
    handler = getattr(manager, "_dataset_file_handler", None)
    if handler is not None:
        handler.flush()

    successful = manager.exported_successful_episode_count
    failed = manager.exported_failed_episode_count
    total = successful + failed

    from pathlib import Path as _Path
    dataset_path = _Path(args_cli.dataset_file).resolve()
    size = dataset_path.stat().st_size if dataset_path.exists() else 0

    print(
        f"[DATASET] reset export complete: episodes={total} "
        f"successful={successful} other={failed} "
        f"bytes={size}",
        flush=True,
    )
        ...
        with contextlib.suppress(KeyboardInterrupt), torch.inference_mode():
            while simulation_app.is_running():
                ...
                if running_recording_instance:
                    ...
                    if subtasks is not None and "obv" in locals():
                        if subtasks == {}:
...
def main() -> None:
    ...
    # Newton 최종 모델의 body, shape, contact pair를 확인한다.
    try:
        ...
        masses = host(model.body_mass)
        inv_masses = host(model.body_inv_mass)
        body_flags = host(model.body_flags)
        shape_body = host(model.shape_body)
        shape_flags = host(model.shape_flags)
        shape_groups = host(model.shape_collision_group)
        shape_types = host(model.shape_type)
        contact_pairs = host(model.shape_contact_pairs)

        state = SimulationManager.get_state_0()
        body_q = host(state.body_q)
        shape_xform = host(model.shape_transform)
        shape_scale = host(model.shape_scale)

        for body_id in (46, 47):
            print(
                "[NEWTON-COLLIDER-POSE] "
                f"body={body_id} pose={body_q[body_id].tolist()}",
                flush=True,
            )
        ...
        for shape_id in (
            ...
            print(
                "[NEWTON-COLLIDER-POSE] "
                f"shape={shape_id} "
                f"body={int(shape_body[shape_id])} "
                f"local={shape_xform[shape_id].tolist()} "
                f"scale={shape_scale[shape_id].tolist()}",
                flush=True,
            )

        print(
            "[NEWTON-COLLISION-DETAIL] "
            f"bodies={model.body_count} "
            f"shapes={model.shape_count} "
            f"shape_contact_pair_count="
            f"{model.shape_contact_pair_count}",
            flush=True,
        )

        print("[NEWTON-COLLISION-DETAIL] bodies:", flush=True)
        for body_id in range(model.body_count):
            shapes = model.body_shapes.get(body_id, [])
            mass = None if masses is None else float(masses[body_id])
            inv_mass = (
                None
                if inv_masses is None
                else float(inv_masses[body_id])
            )
            flags = (
                None
                if body_flags is None
                else int(body_flags[body_id])
            )
            label = (
                model.body_label[body_id]
            ...
            print(
                f"  body={body_id} "
                f"label={label!r} "
                f"mass={mass} inv_mass={inv_mass} "
                f"flags={flags} shapes={list(shapes)}",
                flush=True,
            )
        ...
        for shape_id in range(model.shape_count):
            ...
            is_static = (
                ...
                or (
                    ...
                    and float(masses[body_id]) == 0.0
                )
            )

            if not (is_static or body_id in (46, 47)):
                continue

            label = (
                model.shape_label[shape_id]
                if shape_id < len(model.shape_label)
                else ""
            )
            flags = (
                None
                if shape_flags is None
                else int(shape_flags[shape_id])
            )
            group = (
                None
                if shape_groups is None
                else int(shape_groups[shape_id])
            )
            shape_type = (
                None
                if shape_types is None
                else int(shape_types[shape_id])
            )

            print(
                f"  shape={shape_id} body={body_id} "
                f"label={label!r} type={shape_type} "
                f"flags={flags} group={group}",
                flush=True,
            )

        body_pair_counts = Counter()

        if contact_pairs is not None and shape_body is not None:
            for shape_a, shape_b in contact_pairs:
                body_a = int(shape_body[int(shape_a)])
                body_b = int(shape_body[int(shape_b)])
                pair = tuple(sorted((body_a, body_b)))
                body_pair_counts[pair] += 1

        print(
            "[NEWTON-COLLISION-DETAIL] "
            "contact body pairs involving world/static:",
            flush=True,
        )

        for (body_a, body_b), count in sorted(
            body_pair_counts.items()
        ):
            static_a = (
                body_a == -1
                or (
        ...
        print(
            f"[NEWTON-COLLISION-DETAIL] failed: {exc!r}",
            flush=True,
        )

    # Run simulation loop and always flush/close the HDF5 recorder.
    current_recorded_demo_count = 0
    try:
        ...
        print("[INFO]: Recording interrupted; flushing dataset...", flush=True)
    finally:
        env.close()

    print(
        f"Recording session completed with "
        f"{current_recorded_demo_count} successful demonstrations"
    )
