...
class LocomanipulationG1EnvCfg(ManagerBasedRLEnvCfg):
    ...
    sim: SimulationCfg = SimulationCfg(
        physics=PhysicsCfg().newton_mjwarp,
    )
    ...
    def __post_init__(self):
        """Post initialization."""
        # 하체는 기존 RL 정책이 학습한 G1 actuator 설정을 유지한다.
        # Newton 공식 position-drive 값은 상체에만 적용한다.
        for actuator_name in ("waist", "arms", "hands"):
            actuator = self.scene.robot.actuators[actuator_name]
            actuator.stiffness = 500.0
            actuator.damping = 10.0

        # Newton에서는 Pink IK의 PhysX식 중력보상을 사용하지 않는다.
