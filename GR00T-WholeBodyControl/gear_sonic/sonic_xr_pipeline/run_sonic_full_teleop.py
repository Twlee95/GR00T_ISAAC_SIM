...
def _parse_live_args() -> argparse.Namespace:
    ...
    # Official outer pipeline owns XR arguments.
...
def _create_live_manager_env(config, device, args_cli):
    """Create SONIC with official G1 visual assets and LivePICO."""

    from gear_sonic.sonic_xr_pipeline.sonic_official_asset_bridge import install
    from gear_sonic.sonic_xr_pipeline.sonic_official_scene_bridge import install as install_scene
...
print("[SONIC-RUNNER] entering SONIC controller", flush=True)

controller_path = (
    "/workspace/wbc/gear_sonic/sonic_xr_pipeline/"
    "sonic_controller_clean.py"
)
main_name = chr(95) * 2 + "main" + chr(95) * 2
name_key = chr(95) * 2 + "name" + chr(95) * 2
file_key = chr(95) * 2 + "file" + chr(95) * 2

controller_scope = {
    name_key: main_name,
    file_key: controller_path,
}
controller_code = compile(
    Path(controller_path).read_text(),
    controller_path,
    "exec",
)
exec(controller_code, controller_scope)
