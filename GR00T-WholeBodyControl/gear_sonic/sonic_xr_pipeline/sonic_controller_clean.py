...
def main(override_config: omegaconf.OmegaConf):
    ...
    # The copied official pipeline owns AppLauncher and XR.
    import argparse
    import builtins
    from isaaclab.app import AppLauncher

    parser = argparse.ArgumentParser(description="SONIC controller")
    ...
    if hasattr(builtins, "SONIC_XR_SIMULATION_APP"):
        simulation_app = builtins.SONIC_XR_SIMULATION_APP
        print("[SONIC-XR] using copied official AppLauncher", flush=True)
    else:
    ...
    print(f"[XR-DIAG] robot_valid={robot_prim.IsValid()} active={robot_prim.IsActive() if robot_prim.IsValid() else None}", flush=True)
    if robot_prim.IsValid():
        visibility = UsdGeom.Imageable(robot_prim).ComputeVisibility()
        matrix = UsdGeom.Xformable(robot_prim).ComputeLocalToWorldTransform(Usd.TimeCode.Default())
        print(f"[XR-DIAG] visibility={visibility} position={matrix.ExtractTranslation()}", flush=True)
        print(f"[XR-DIAG] children={[c.GetName() for c in robot_prim.GetChildren()]}", flush=True)
    matches = [str(p.GetPath()) for p in stage.Traverse() if "robot" in str(p.GetPath()).lower() or "g1" in str(p.GetPath()).lower()]
    print(f"[XR-DIAG] matching_prims={matches[:80]}", flush=True)
