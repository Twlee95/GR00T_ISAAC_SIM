...
def contact_forces(env, sensor_cfg: SceneEntityCfg) -> torch.Tensor:
    """손 링크별 접촉 합력 (world frame)."""
    sensor = env.scene.sensors[sensor_cfg.name]
    return sensor.data.net_forces_w.torch.view(env.num_envs, -1)
...
class LocomanipulationG1SceneCfg(InteractiveSceneCfg):
    ...
    contact_left_hand = ContactSensorCfg(
        prim_path="{ENV_REGEX_NS}/Robot/left_hand/.*_link",
        history_length=0,
    )
    contact_right_hand = ContactSensorCfg(
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        left_hand_contact_force = ObsTerm(
            func=contact_forces,
            params={"sensor_cfg": SceneEntityCfg("contact_left_hand")},
        )
        right_hand_contact_force = ObsTerm(
            func=contact_forces,
            params={"sensor_cfg": SceneEntityCfg("contact_right_hand")},
        )
        object = ObsTerm(
