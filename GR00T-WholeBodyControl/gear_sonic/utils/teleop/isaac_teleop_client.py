...
class IsaacTeleopClient:
    ...
    def _clear_trackers_and_session_ref(self) -> None:
        ...
        if self._cloudxr_launcher is not None:
            try:
                ...
                pass
            try:
                _proc = getattr(self._cloudxr_launcher, "_runtime_proc", None)
                if _proc is not None and _proc.poll() is None:
                    import os as _os, signal as _sig, time as _t
                    _os.kill(_proc.pid, _sig.SIGKILL)
                    _t.sleep(1.0)
                    print(f"IsaacTeleopClient: force-killed stale runtime pid={_proc.pid}")
            except Exception:
                pass
    ...
    def start_streaming(self) -> None:
        ...
        try:
            ...
            import os as _os0
            if self._cloudxr_launcher is not None:
                print('IsaacTeleopClient: reusing existing CloudXR launcher/runtime.')
            elif _os0.environ.get('SKIP_CXR_LAUNCH') == '1':
                print('IsaacTeleopClient: SKIP_CXR_LAUNCH=1, attaching to existing runtime.')
            else:
            ...
            if "Failed to get OpenXR system" in str(e) or "OpenXR" in str(e):
                self._deviceio_session = None
                print(f"IsaacTeleopClient: no XR session yet ({e}); keeping CloudXR runtime alive for headset connect.")
                if self._cloudxr_launcher is None:
                    try:
                        ...
                        print(f"IsaacTeleopClient: launcher revive failed ({le}).")
            else:
                raise
