...
class G1Deploy {
  ...
  public:
    ...
    G1Deploy(
      ...
      std::cout << "[DEBUG] Before MotionSwitcherClient" << std::endl;
      msc_ = std::make_unique<unitree::robot::b2::MotionSwitcherClient>();
      std::cout << "[DEBUG] MotionSwitcherClient created" << std::endl;
      msc_->SetTimeout(5.0f);
      std::cout << "[DEBUG] MotionSwitcherClient timeout set" << std::endl;
      msc_->Init();
      std::cout << "[DEBUG] MotionSwitcherClient init done" << std::endl;
      std::string form, name;
      std::cout << "[DEBUG] Before CheckMode" << std::endl;
      while (msc_->CheckMode(form, name), !name.empty()) {
        std::cout << "[DEBUG] Motion mode: form=" << form << ", name=" << name << std::endl;
        if (msc_->ReleaseMode()) std::cout << "Failed to switch to Release Mode\n";
        sleep(5);
      }
      std::cout << "[DEBUG] After MotionSwitcherClient block" << std::endl;      
